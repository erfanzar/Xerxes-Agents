...
description: Use when a fetch fails — 403/429, paywall, WAF or bot wall — by working an archive-first recovery ladder with provenance discipline.
version: 1.0.0
author: Hermes Agent (adapted for Xerxes)
platforms: [linux, macos, windows]
tags: [research, archives, wayback, paywall, waf, fallback]
source: https://raw.githubusercontent.com/NousResearch/hermes-agent/main/skills/web/blocked-page-recovery/SKILL.md
---

# Blocked-Page Recovery

When a page won't fetch — 403/429, a Cloudflare "Just a moment..." wall, a
paywall, or a bot-detection interstitial — don't give up and don't loop on the
same URL. Third-party services often hold a **copy** of the page. Work down
this ladder, cheapest first:
...
snapshot is context, not an answer — say so explicitly and note its age. For
the citation format itself, see the `grounded-citations` skill.

## Route 1 — Wayback Machine (best provenance, try first)

```bash
# Discovery: returns the closest snapshot URL + timestamp as JSON
...
## Route 2 — archive.today (paywalls, deleted content)
...
rate-limit HTML that looks like success to a size check alone.

## Route 3 — Jina Reader (requires JINA_API_KEY)
...
- An RSS/Atom feed (`/feed`, `/rss`, `<link rel="alternate">` in any copy you
  did recover)
...
- **AMP caches** (`*.cdn.ampproject.org`) mostly return a ~300-byte
  `<title>Redirecting</title>` meta-refresh stub pointing back at the original
  (blocked) URL. Treating that as success creates a fetch loop.
...
Detection heuristics: body under a per-route byte floor; meta-refresh or
JS-redirect stubs whose target is the original host; interstitial titles
("Just a moment", "Redirecting", "Google Search", "Attention Required").
...
- [ ] The recovered body contains the page's actual expected content, not a
      redirect stub or rate-limit HTML.
- [ ] The citation states the provenance and, for snapshots, the archive
      date.
- [ ] Stale-data risk is called out when the user needs current facts.
- [ ] No credentials were sent through a proxy relay.

---

Adapted from the `blocked-page-recovery` skill in [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) (MIT License), copyright Hermes Agent.
