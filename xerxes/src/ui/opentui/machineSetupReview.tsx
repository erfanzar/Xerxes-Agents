...
const integrations = [
  ...
  { name: 'Providers and API credentials', details: 'Keep remote saved profiles, authentication or environment credentials, or review a supported local profile in the next step. With local access, provider requests run on this workstation; code and tools run remotely. Keys are not copied. Local grants stay in memory and expire; the remote task saves only its provider requirement.', setup: 'The next review names the local profile, model, credential source, limits and destination before authorization. After connecting, /provider can explicitly switch the task to a remote profile. /providers lists remote profiles.' },
  ...
  { name: 'Models and reasoning', details: 'The next review uses the selected local profile’s configured default model. Remote sessions retain their saved model and reasoning choices. A local grant covers only its reviewed model; changing providers requires an explicit choice.', setup: 'For another local model, configure its local profile before connecting and review a new grant. In the remote task, /model with an explicit remote profile or /provider selects remote credentials. /reasoning controls supported effort levels. Selection alone does not prove connectivity.' },
