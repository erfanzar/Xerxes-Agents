...
| `usage.report`          | `{ session_key?, refresh? }`                                 | `{ ok, fetched_at, profiles[], session? }`                  | Session statistics plus every imported provider profile: `status` `ok` (plan `windows[]` with `usedPercent`/`resetsAt`/`resetAfterSeconds`, or an API `balance`), `unsupported`, or `error` with `message`. Cached per credential for 60s; `refresh` bypasses it. |
