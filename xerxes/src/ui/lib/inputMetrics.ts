...
// The controller's visual-line model also delegates to wrap-ansi, so any
// drift between the two algorithms parks the
 ...
 *
 * IMPORTANT: this MUST stay in lock-step with the composer's wrapped layout.
 * Any divergence parks the
...
export function cursorLayout(value: string, cursor: number, cols: number) {
  ...
  // above already matches what the composer will render. Pushing the
