...
const THINKING_STATUS_WORD_RE = new RegExp(`\\b(?:${VERBS.join('|')})\\b`, 'i')
const THINKING_STATUS_CHUNK_RE = new RegExp(`[^A-Za-z\n]+\\s*(?:${VERBS.join('|')})\\.{0,3}\\s*`, 'giu')

export const cleanThinkingText = (reasoning: string) =>
  reasoning
    .split('\n')
    .map(line => (THINKING_STATUS_WORD_RE.test(line) ? line.replace(THINKING_STATUS_CHUNK_RE, '') : line).trim())
...
export const parseToolTrailResultLine = (line: string) => {
  ...
  // Context is serialized inside ("..."). A shell command or search pattern
  // may itself contain " :: ", so begin looking after the contextual call's
  // closing delimiter instead of splitting the first occurrence blindly.
  const contextualClose = body.indexOf('")')
  const sep = body.indexOf(' :: ', contextualClose >= 0 ? contextualClose + 2 : 0)
...
export const inlineToolDisplay = (call: string) => {
  const { label } = splitToolDuration(call)
  const contextual = label.match(/^(.*?)\("(.*)"\)$/)

  return contextual ? `${contextual[1]} ${contextual[2]}`.trim() : label
}
