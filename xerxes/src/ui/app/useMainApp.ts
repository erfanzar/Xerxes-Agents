...
export function useMainApp(gw: GatewayClient) {
  ...
  useEffect(() => {
    ...
    const sync = () => setTerminalCols(stdout.columns ?? 80)
  ...
  useEffect(() => {
    selection.setSelectionBgColor(theme.color.selectionBg)
  }, [selection, theme.color.selectionBg])
  ...
  //
  // Subscribe directly via the terminal selection service
  ...
  const visibleHistoryItems = useMemo(() => visibleTranscriptMessages(historyItems), [historyItems])
  const empty = !hasTranscriptContent(historyItems)
  ...
  const detailsLayoutKey = useMemo(() => {
    ...
    const subagents = sectionMode('subagents', ui.detailsMode, ui.sections, ui.detailsModeCommandOverride)
  ...
  const die = useCallback(() => {
    gw.kill('app.die')
    exit()
    // Destroying the OpenTUI renderer does not call process.exit(). Without
    // an explicit exit the Bun process stays
  ...
  useEffect(() => {
    if (ui.busy) {
      ...
      // before the first turn has completed. Capture the duration here
      // (before clearing turnStartedAt) so the "Worked for Xs" trailer has
      // something to read once the turn settles.
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Xerxes'
  )
  ...
  const answerClarify = useCallback(
    (answer: string) => {
      ...
      const isProviderFlow = clarify.source === 'provider'
      ...
      rpc<ClarifyRespondResponse>('clarify.respond', { answer, request_id: clarify.requestId })
        .then(r => {
          ...
          if (answer && !isProviderFlow) {
            if (clarify.toolId) {
              turnController.persistedToolIds.add(clarify.toolId)
            } else {
          ...
          // Provider setup can synchronously emit the next question before
          // the answer RPC resolves. Only close the prompt we answered; this
          // preserves the new provider step instead of leaving a blank TUI.
          if (clearClarifyOverlay(clarify.requestId) && answer) {
  ...
  const { dispatchQueuedSubmission, dispatchSubmission, send, submit } = useSubmission({
    appendMessage,
    composerActions,
    composerRefs,
    composerState,
  ...
  const answerApproval = useCallback(
    (choice: string) => {
      ...
      return respondWith('approval.respond', { choice, request_id: requestId, session_id: ui.sid }, () => {
        clearApprovalOverlay(requestId)
  ...
  const cwd = ui.info?.cwd || process.env.XERXES_PROJECT_DIR || process.env.XERXES_CWD || process.cwd()
  ...
  const appStatus = useMemo(
    () => ({
      ...
      statusColor: statusColorOf(ui.status, theme.color),
      stickyPrompt,
