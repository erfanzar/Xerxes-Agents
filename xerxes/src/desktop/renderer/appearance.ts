...
/** A fresh install's interface text size (XS). */
...
export function parseAppearance(value: unknown): { theme: 'system' | 'dark' | 'light'; font: FontSize } {
  ...
  return { theme: record.theme === 'dark' || record.theme === 'light' ? record.theme : 'system', font: font ?? DEFAULT_FONT_SIZE }
}
export function saveAppearance(): void {
  try { localStorage.setItem(APPEARANCE_KEY, JSON.stringify({ theme: document.documentElement.getAttribute('data-user-theme') ?? 'system', font: document.documentElement.getAttribute('data-font') ?? DEFAULT_FONT_SIZE })) } catch { /* Keep the current window usable when persistence is unavailable. */ }
}
