...
export const COMMAND_REGISTRY: readonly CommandDefinition[] = Object.freeze([
  ...
  command('rollback', 'Preview or restore filesystem to a snapshot', 'snapshots', { argsHint: '[diff <id>|apply <id> <revision>|<id>]' }),
  ...
  command('config', 'Inspect runtime config or edit agents/MCP/LSP settings', 'config', { argsHint: '[agents|mcp|lsp]' }),
  ...
  command('mcp', 'Inspect MCP connection health or reconnect a server', 'tools', { argsHint: '[status|reconnect <name>]' }),
  ...
  command('context', 'Inspect context layers, pin memory and exclude stale recall', 'info'),
