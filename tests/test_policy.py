...
"""Tests for xerxes_agent.policy — tool policy allow/deny enforcement."""

import pytest
from xerxes_agent.security.policy import PolicyAction, PolicyEngine, ToolPolicy, ToolPolicyViolation
...
class TestPolicyEngine:
    ...
    def test_agent_policy_overrides_global(self):
        ...
        # Global denies execute_shell
        assert engine.check("execute_shell") == PolicyAction.DENY
        # Agent-specific allows it
        assert engine.check("execute_shell", agent_id="coder") == PolicyAction.ALLOW
        # Different agent falls back to global
    ...
    def test_enforce_passes_on_allow(self):
        ...
        engine.enforce("search")  # Should not raise
