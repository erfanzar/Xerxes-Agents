...
Maps the typed event hierarchy from :mod:`xerxes_agent.audit.events` into
...
Usage::
    ...
    from xerxes_agent.audit import AuditEmitter, CompositeCollector, InMemoryCollector
    from xerxes_agent.audit.otel_exporter import OTelCollector
...
class OTelCollector:
    ...
    def _on_turn_start(self, event: TurnStartEvent) -> None:
        """Open a ``xerxes_agent.turn`` span and index it by ``turn_id`` for later close."""
        if self._tracer is None or not event.turn_id:
            self._noop_log.append({"name": "turn", "attributes": _clean_attrs(event.to_dict())})
            return
        span = self._tracer.start_span(
            name="xerxes_agent.turn",
            attributes=_clean_attrs(
                {
                    "xerxes_agent.turn_id": event.turn_id,
                    "xerxes_agent.agent_id": event.agent_id,
                    "xerxes_agent.session_id": event.session_id,
                    "xerxes_agent.prompt_preview": event.prompt_preview,
    ...
    def _on_turn_end(self, event: TurnEndEvent) -> None:
        """Set ``xerxes_agent.function_calls_count`` and close the matching turn span."""
        ...
        if span is None:
            return
        try:
            span.set_attribute("xerxes_agent.function_calls_count", event.function_calls_count)
            span.end()
        except Exception:
            pass
