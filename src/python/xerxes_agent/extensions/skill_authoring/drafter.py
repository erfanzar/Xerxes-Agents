...
def render_skill_template(
    ...
    *,
    ...
    2. ``# When to use``: derived from the originating user prompt.
    3. ``# Procedure``: numbered list of tool invocations (success-only).
    4. ``# Pitfalls``: any failures observed before recovery, plus retries.
    5. ``# Verification``: assertions over the final tool sequence shape.
