...
def _daemon_command() -> str:
    return f"{_python_path()} -m xerxes_agent.daemon"
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>xerxes_agent.daemon</string>
    </array>
...
[Service]
Type=simple
ExecStart={python} -m xerxes_agent.daemon
...
def install(project_dir: str = "", log_dir: str = "") -> str:
    ...
    if not log_dir:
        from xerxes_agent.core.paths import xerxes_subdir
    ...
    if system == "Darwin":
        ...
        return f"Unsupported platform: {system}. Run manually with `python -m xerxes_agent.daemon`."
