...
Typical usage example:
    from xerxes_agent import Xerxes, Agent
    ...
    xerxes_agent.register_agent(agent)

    # Generate a response (streaming)
    for chunk in xerxes_agent.run(prompt="Hello!"):
        if chunk.content:
            print(chunk.content, end="")

    # Generate a response (non-streaming)
    result = xerxes_agent.run(prompt="Hello!", stream=False)
...
from xerxes_agent.types.function_execution_types import ReinvokeSignal
from xerxes_agent.types.messages import ChatMessage, MessagesHistory, SystemMessage, UserMessage
...
class Xerxes:
    ...
    Example:
        ...
        >>> async for chunk in await xerxes_agent.create_response(prompt="Hi"):
        ...     print(chunk.content, end="")
    """
    ...
    def create_query_engine(
        ...
        **config_kwargs: tp.Any,
        ...
        Example:
            >>> engine = xerxes_agent.create_query_engine(model="gpt-4o")
    ...
    def create_subagent_manager(
        ...
        Example:
            >>> mgr = xerxes_agent.create_subagent_manager()
            >>> from xerxes_agent.agents import get_agent_definition
            >>> task = mgr.spawn(
    ...
    def _format_section(
        ...
        Example:
            >>> xerxes_agent._format_section("RULES:", ["Be helpful", "Be concise"])
            'RULES:\\n  - Be helpful\\n  - Be concise'
        """
        if not content:
            return None
    ...
    def _extract_from_markdown(self, content: str, field: str) -> list[str]:
        ...
        Example:
            >>> content = '```tool_call\\n{"name": "func"}\\n```'
            >>> xerxes_agent._extract_from_markdown(content, "tool_call")
            ['{"name": "func"}']
        """
        pattern = rf"```{field}\s*\n(.*?)\n```"
    ...
    def manage_messages(
        ...
        Example:
            >>> messages = xerxes_agent.manage_messages(
            ...     agent=my_agent,
            ...     prompt="Hello",
    ...
    async def create_response(
        ...
        Example:
            >>> response = await xerxes_agent.create_response(
        ...
        try:
            ...
            json.dump({"key": prompt_str}, open("debug_prompt.json", "w"), indent=2)
    ...
    def run(
        ...
        Example:
            >>>
            >>> for chunk in xerxes_agent.run(prompt="Hello", stream=True):
            ...     if chunk.content:
            ...         print(chunk.content, end="")
            >>>
            >>>
    ...
    def thread_run(
        ...
        Example:
            >>> buffer, thread = xerxes_agent.thread_run(prompt="Hello")
    ...
    async def athread_run(
        ...
        Example:
            >>> buffer, task = await xerxes_agent.athread_run(prompt="Hello")
    ...
    def create_ui(self, target_agent: Agent = None):
        ...
        Launches a graphical user interface for interacting with the Xerxes
        ...
        Example:
            >>> xerxes = Xerxes(llm=my_llm)
            >>> xerxes_agent.register_agent(my_agent)
            >>> xerxes_agent.create_ui()  # Launches interactive UI
