...
"""Chainlit application entry point for Xerxes UI.
...
Example:
    ...
    >>> xerxes = Xerxes(llm=my_llm, agents=[my_agent])
...
try:
    ...
    from xerxes_agent.ui.themes import APP_TITLE, setup_chainlit_theme
...
_EXECUTOR_CONFIG_KEY = "_xerxes_executor_config"
...
def _set_executor_config(executor, agent) -> None:
    ...
    Args:
        executor: The Xerxes executor instance to store.
...
class ChainlitLauncher:
    ...
    Attributes:
        executor: The Xerxes executor instance for processing conversations.
    ...
    def __init__(
        ...
        Args:
            executor: Xerxes instance or Cortex component for managing conversations.
            agent: Optional agent configuration for specialized behavior.
        """
...
def launch_application(
    ...
    **kwargs,
    ...
    Args:
        executor: Xerxes instance or Cortex component for managing conversations.
            Supported types include Xerxes, CortexAgent, CortexTask, Cortex,
    ...
    Example:
        >>> app = launch_application(executor=xerxes, agent=my_agent)
...
if "chainlit" in sys.modules or os.environ.get("CHAINLIT_ROOT_PATH"):
    ...
    from xerxes_agent.types.messages import MessagesHistory

    try:
        from .helpers import process_message_chainlit
    except ImportError:
        from xerxes_agent.ui.helpers import process_message_chainlit
    ...
    async def on_chat_start():
        ...
        cl.user_session.set("xerxes_msgs", MessagesHistory(messages=[]))
    ...
    async def on_regenerate(action: cl.Action):
        ...
        """
        xerxes_msgs = cl.user_session.get("xerxes_msgs")

        if not xerxes_msgs or len(xerxes_msgs.messages) < 2:
            await cl.Message(content="Nothing to regenerate.").send()
            return

        xerxes_msgs.messages.pop()
        last_user_msg = xerxes_msgs.messages[-1].content
        ...
        updated_msgs = await process_message_chainlit(
            ...
            xerxes_msgs=xerxes_msgs,
            executor=executor,
            agent=agent,
        )

        cl.user_session.set("xerxes_msgs", updated_msgs)
    ...
    async def on_chat_end():
        ...
        """
        cl.user_session.set("xerxes_msgs", None)
