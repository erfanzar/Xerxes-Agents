...
class Xerxes:
    ...
    @staticmethod
    def _is_generated_system_prompt(content: str) -> bool:
        """Return true for system prompts generated by :meth:`manage_messages`."""
        text = content.lstrip()
        if text.startswith("[Identity]\n- You are Xerxes"):
            return True
        return (
            "After a function returns a result, use that result to continue the task and answer the user." in text
            and "Do not repeat the same function call with the same arguments" in text
        )
    ...
    def _merge_system_history(
        ...
        for message in messages.messages:
            if isinstance(message, SystemMessage):
                ...
                if system_text:
                    if final_system_content.strip() and self._is_generated_system_prompt(system_text):
                        continue
