...
def _build_system_prompt(context: dict[str, Any], extra: str = "") -> str:
    ...
    parts = [
        ...
        "exec_command, write_stdin, list_terminal_sessions, close_terminal_session, apply_patch,",
        "web.search_query, web.open, DuckDuckGoSearch, APIClient, URLAnalyzer,",
        ...
        "4. Need to edit a file with a known diff? → apply_patch(patch=...)",
        "   Need a simple text replacement? → FileEditTool(file_path=..., old_string=..., new_string=...)",
        "5. Need to run a command? → exec_command(cmd=..., yield_time_ms=1000).",
        "   If it is still running, poll with write_stdin(session_id=..., chars=''), send input with chars,",
        "   interrupt with interrupt=true, and clean up with close_terminal_session(session_id=...).",
        "   Use terminal sessions for both short and long commands; do not use blocking shell tools.",
        "6. Need to find files? → GlobTool(pattern=...)",
        "7. Need to search code? → GrepTool(pattern=...)",
        "8. Need web info? → web.search_query(q=...) when available, otherwise DuckDuckGoSearch(query=...).",
        "9. Need to list a directory? → ListDir(directory_path=...)",
        "",
        "# Context headroom",
        "- Oversized tool results are stored in project agent memory and replaced with a bounded Headroom preview.",
        "- Treat `[Large tool result stored outside model context]` as a valid tool result, not a failure.",
        '- If the preview is insufficient, read the full artifact with `agent_memory_read("project", path)` from the pointer.',
        "- Do not rerun noisy commands just to recover output that is already stored in project memory.",
        "",
        "# Interaction modes",
        ...
        "- objective: hard-goal loop for measurable outcomes. Use it when the user asks you to keep trying until tests, benchmarks, or explicit acceptance criteria pass.",
        "Switch modes with SetInteractionModeTool(mode=...).",
        "Enter objective mode when success can be verified by a command, benchmark, test, or concrete artifact.",
        "In objective mode, keep a compact ledger of objective, acceptance criteria, best result, failed attempts, current hypothesis, and next verification command.",
        ...
        "",
        "# Web search via terminal fallback",
        "If the normal web-search tools are unavailable or return no useful results,",
        "fall back to exec_command with curl and a real browser User-Agent.",
        "",
