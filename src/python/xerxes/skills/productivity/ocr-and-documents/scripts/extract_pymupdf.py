...
"""Quick-and-dirty PDF extraction using PyMuPDF.

CLI sub-commands print text, markdown, tables, images, or metadata for the
provided PDF ``path``.
"""
...
def extract_text(path, pages=None):
    """Print extracted text from ``path`` for ``pages`` (or all pages)."""
    import pymupdf
...
def extract_markdown(path, pages=None):
    """Print a markdown rendering of ``path`` via ``pymupdf4llm.to_markdown``."""
    import pymupdf4llm
...
def extract_tables(path):
    """Print every detected table in ``path`` formatted as GitHub-flavoured markdown."""
    import pymupdf
...
def extract_images(path, output_dir):
    """Save every embedded image in ``path`` as a PNG under ``output_dir``."""
...
def show_metadata(path):
    """Print the document metadata for ``path`` as indented JSON."""
    import pymupdf
