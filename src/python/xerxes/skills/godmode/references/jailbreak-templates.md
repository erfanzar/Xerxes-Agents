...
## 5. GODMODE FAST — Xerxes 4 405B (Zero Refusal)

**Model:** `nousresearch/Xerxes-4-405b`
**Strategy:** Xerxes Models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Xerxes Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.xerxes/config.yaml`:
