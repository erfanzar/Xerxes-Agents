...
every significant decision and transition in a Xerxes agent run. All events
...
Example:
    Creating and serializing a tool-call event::
        ...
        event = ToolCallAttemptEvent(
            ...
            arguments_preview='{"query": "xerxes docs"}',
            agent_id="agent-1",
            turn_id="abc123",
        )
        print(event.to_json())
...
class AuditEvent:
    ...
    Every audit event in the Xerxes audit system inherits from this
    ...
    Attributes:
        ...
        session_id: Optional session-level identifier, typically stamped
            by the :class:`~xerxes.audit.emitter.AuditEmitter`.
...
class SkillUsedEvent(AuditEvent):
    """Emitted whenever an authored skill is invoked by the agent.

    Captures every skill activation so that downstream analytics can
    track invocation count, success rate, and latency per skill.

    Attributes:
        event_type: Fixed to ``"skill_used"``.
        skill_name: Name of the skill that was used.
        version: Skill version (semver) at the time of use.
        outcome: ``"success"`` / ``"failure"`` / ``"unknown"``.
        duration_ms: Wall-clock time spent in the skill's procedure.
        triggered_automatically: ``True`` when the skill matcher chose
            it; ``False`` when the user explicitly requested it.
    """
...
class SkillAuthoredEvent(AuditEvent):
    """Emitted when the autonomous skill authoring loop drafts a skill.

    Attributes:
        event_type: Fixed to ``"skill_authored"``.
        skill_name: Name of the newly authored skill.
        version: Initial version (e.g. ``"0.1.0"``).
        source_path: Filesystem path where the SKILL.md was written.
        tool_count: Number of tool calls captured in the source candidate.
        unique_tools: Distinct tool names in the candidate.
        confirmed_by_user: ``True`` when the user accepted the suggestion.
    """
...
class SkillFeedbackEvent(AuditEvent):
    """User-supplied (or system-derived) feedback signal for a skill.

    Attributes:
        event_type: Fixed to ``"skill_feedback"``.
        skill_name: Name of the skill being rated.
        rating: ``"good"`` / ``"bad"`` / ``"neutral"``.
        reason: Optional free-text explanation.
        source: ``"user"`` (explicit feedback) or ``"system"``
            (e.g. derived from retries / verification failures).
    """
...
class AgentSwitchEvent(AuditEvent):
    """Emitted when the orchestrator switches active agents.

    Attributes:
        event_type: Fixed to ``"agent_switch"``.
