...
"""Unix domain socket channel for local ``calute send`` commands.

Provides a newline-delimited JSON protocol over a Unix socket at
``~/.calute/daemon/calute.sock`` for fast, auth-free local task
    ...
    async def _handle_client(
        ...
        try:
            while True:
                ...
                # Close after response for submit (one-shot pattern).
                if method == "submit":
                    break
