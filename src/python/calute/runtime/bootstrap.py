...
def bootstrap(
    ...
    3. Load CALUTE.md project context.
    ...
    Args:
        ...
        include_calute_md: Whether to load CALUTE.md files.
    ...
    t0 = time.monotonic()
    calute_md = ""
    if include_calute_md:
        calute_md = _load_calute_md(working_dir)
        result.context["calute_md"] = calute_md
    result.stages.append(
        BootstrapStage(
            name="calute_md",
            status="ok" if calute_md else "skipped",
            detail=f"{len(calute_md)} chars" if calute_md else "No CALUTE.md found",
            duration_ms=(time.monotonic() - t0) * 1000,
        )
    )
...
def _load_calute_md(cwd: Path) -> str:
    """Load CALUTE.md from cwd/parents and ~/.calute/CALUTE.md."""
    parts = []

    global_md = Path.home() / ".calute" / "CALUTE.md"
    if global_md.exists():
        try:
            parts.append(f"[Global CALUTE.md]\n{global_md.read_text()}")
        except Exception:
            pass

    p = cwd
    for _ in range(10):
        candidate = p / "CALUTE.md"
        if candidate.exists():
            try:
                parts.append(f"[Project CALUTE.md: {candidate}]\n{candidate.read_text()}")
            except Exception:
                pass
            break
        parent = p.parent
...
def _build_system_prompt(context: dict[str, Any], extra: str = "") -> str:
    ...
    calute_md = context.get("calute_md", "")
    if calute_md:
        parts.extend(["", "# Project Context", calute_md])
