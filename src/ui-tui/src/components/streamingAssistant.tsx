...
export const LiveAgentPanel = memo(function LiveAgentPanel() {
  const ui = useStore($uiState)
  ...
  const finished = subagents.filter(s => s.status !== 'running' && s.status !== 'queued')
  const rows = [...running, ...finished].slice(0, 8)
  const totalTools = subagents.reduce((sum, s) => sum + (s.toolCount ?? 0), 0)
  const done = finished.length
  const allDone = done === subagents.length

  return (
    <Box flexDirection="column" marginBottom={1}>
      <Text color={ui.theme.color.muted}>
        <Text color={allDone ? ui.theme.color.accent : ui.theme.color.system}>{allDone ? '✓' : '⬡'} </Text>
        <Text bold color={ui.theme.color.text}>
          {subagents.length} agents
        </Text>{' '}
        {done}/{subagents.length} · {totalTools} calls{' '}
      ...
      <Box flexDirection="column" marginLeft={2}>
        {rows.map(agent => {
          ...
          const icon = active ? '◌' : failed ? '✗' : '✓'
          const color = active ? ui.theme.color.warn : failed ? ui.theme.color.error : ui.theme.color.accent
          const detail = latestAgentDetail(agent)
          const elapsedSeconds = subagentElapsedSeconds(agent)
          const elapsed = elapsedSeconds === null ? '' : fmtDuration(elapsedSeconds)

          return (
            <Text color={active ? ui.theme.color.text : ui.theme.color.muted} dim={!active} key={agent.id}>
              <Text color={color}>{icon} </Text>
              <Text color={ui.theme.color.system}>{agent.goal.slice(0, 16).padEnd(16)}</Text>{' '}
              <Text color={ui.theme.color.muted}>{String(agent.toolCount ?? 0).padStart(4)}</Text> {detail}
              {elapsed ? (
                <Text color={ui.theme.color.muted} dim>
                  {' '}
                  {elapsed}
        ...
        {subagents.length > rows.length && (
          <Text color={ui.theme.color.muted} dim>
            ... +{subagents.length - rows.length} more
          </Text>
        )}
      </Box>
    </Box>
...
const latestAgentDetail = (agent: { notes: string[]; summary?: string; thinking: string[]; tools: string[] }) =>
  (agent.summary || agent.notes.at(-1) || agent.tools.at(-1) || agent.thinking.at(-1) || 'working')
    .replace(/\s+/g, ' ')
    .slice(0, 72)
