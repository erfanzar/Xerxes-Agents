import { Box, Link, stringWidth, Text } from '@xerxes/ink'
...
function DiffFence({ cols, lines, t }: { cols?: number; lines: string[]; t: Theme }) {
  const body = lines.filter((line, i) => line !== '' || i < lines.length - 1)
  const shown = body.slice(0, DIFF_PREVIEW_LINES)
  const hidden = Math.max(0, body.length - shown.length)
  const { adds, dels } = diffStats(body)
  const width = Math.max(12, (cols ?? 100) - DIFF_PADDING_LEFT - SAFETY_MARGIN)

  return (
    <Box flexDirection="column" paddingLeft={DIFF_PADDING_LEFT}>
      <Text color={t.color.muted} wrap="truncate-end">
        diff <Text color={t.color.diffAddedWord}>+{adds}</Text> <Text color={t.color.diffRemovedWord}>-{dels}</Text>
        {hidden ? (
          <Text color={t.color.muted}>
            {' '}
            · showing {shown.length}/{body.length} lines
          </Text>
        ) : null}
      </Text>
  ...
  const nodes = useMemo(() => {
    ...
    while (i < lines.length) {
      ...
      if (fence) {
        ...
        if (isDiff) {
          nodes.push(<DiffFence cols={cols} key={key} lines={block} t={t} />)
        ...
        nodes.push(
          <Box flexDirection="column" key={key} paddingLeft={2}>
            {lang && <Text color={t.color.muted}>{'─ ' + lang}</Text>}
