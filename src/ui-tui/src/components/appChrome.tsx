import { Box, type ScrollBoxHandle, stringWidth, Text } from '@xerxes/ink'
...
export function StatusRule({
  ...
  const computedPct = max > 0 ? Math.round((used / max) * 1000) / 10 : undefined
  const contextPct = usage.context_percent ?? computedPct
  const statusModelText = model || '(not set)'
  const interactionMode = mode || 'code'
  const contextText =
    max > 0
      ? `context: ${contextPct != null ? `${contextPct}% ` : ''}(${fmtK(used)}/${fmtK(max)})`
      : used > 0
        ? `context: ${fmtK(used)}`
        : 'context: 0.0% (0/0)'
  const rightWidth = Math.min(stringWidth(contextText), Math.max(0, cols - 4))
  const leftWidth = Math.max(1, cols - rightWidth)
  const activity = busy ? (
  ...
  return (
    <Box height={1}>
      <Box flexShrink={1} overflow="hidden" width={leftWidth}>
        <Text color={t.color.muted}>agent </Text>
        <Text color={busy ? t.color.statusGood : t.color.muted}>
          ({statusModelText} {busy ? '●' : '○'})
        </Text>
        <Text color={t.color.muted}> {cwdLabel}</Text>
        <Text color={t.color.system}> mode: {interactionMode}</Text>
        <Text color={t.color.muted}> think: {modelReasoningEffort || 'off'}</Text>
        {activity ? <Text color={t.color.muted}> </Text> : null}
        {activity}
        {bgCount > 0 ? <Text color={t.color.muted}> {bgCount} bg</Text> : null}
        {liveSessionCount > 0 ? <Text color={t.color.muted}> {statusSessionCountLabel(liveSessionCount)}</Text> : null}
      </Box>

      {rightWidth > 0 ? (
        <Box flexShrink={0} justifyContent="flex-end" width={rightWidth}>
          <Text color={ctxBarColor(contextPct, t)} wrap="truncate-end">
            {contextText}
          </Text>
        </Box>
      ) : null}
