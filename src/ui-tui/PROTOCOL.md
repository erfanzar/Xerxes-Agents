...
This is the frozen contract between the **TypeScript/Ink frontend** (`src/ui-tui/`)
and the **Python daemon** (`python -m xerxes.daemon`). The frontend owns the
screen; the daemon owns sessions, tools, model calls, and command logic. The TS
client is a JSON-RPC peer over a Unix domain socket — it does **not** import any
Python.

The authoritative Python sources for this document are:

- Transport + framing: `src/python/xerxes/daemon/socket_channel.py`
- RPC dispatch + params: `src/python/xerxes/daemon/server.py` (`_handle_rpc`)
- Event payload shapes: `src/python/xerxes/streaming/wire_events.py`
- Connection bootstrap: `src/python/xerxes/tui/engine.py` (`BridgeClient`)
- Build-id / staleness: `src/python/xerxes/daemon/fingerprint.py`

> **Stability:** the event-name map (snake_case ⇄ PascalCase) is asserted by a
> Python test (`tests/.../test_wire_protocol_contract.py`). If you change an
> event name or add an event, update that test **and** `gatewayTypes.ts`.
...
1. Resolve the project dir: `realpath(cwd)` (fallback `abspath`).
2. Compute the per-project socket path (default layout):
   ...
   The daemon config (`$XERXES_HOME/daemon/config.json`) may override
   `socket_path`; the TS client should mirror `BridgeClient._daemon_config()`.
3. Try to `net.connect({ path })`. If it connects, call `runtime.status` and
   verify `daemon_build_id` matches the expected build id; if stale, stop the
   daemon (via its pid file) and relaunch.
4. If not reachable, spawn the daemon and poll `runtime.status` for ≤10s:
   ...
   Spawn detached (`start_new_session`/`detached`), `stdin=/dev/null`,
   `stdout=/dev/null`, capture `stderr` into an in-memory ring (≤200 lines) for
   diagnostics — never write child stderr to the terminal.
...
> (`text_part`, `turn_begin`, `tool_call`, `question_request`, …). It does
> *not* run events through `to_kimi_event_name` — `runtime.py` pushes
> snake_case and `server.py::_broadcast_event` forwards it verbatim. (Only the
> stdio **bridge** — `xerxes.bridge` — converts to PascalCase.) The TS client
> therefore **keys on snake_case** and treats PascalCase as a tolerated alias,
> so the same handler map works if/when it ever attaches to the bridge instead.

---

## 2. Requests (methods the TS client calls)

`params` shapes are from `server.py::_handle_rpc`. Omitted optional params fall
back to per-connection defaults.
...
| `session.open`                       | `{ session_key?, agent_id? }`                  | `{ ok, session }`                                              | Open/attach a session.                                                            |
| `session.list`                       | `{}`                                           | `{ ok, sessions }`                                             | For `/resume` picker.                                                             |
| `session.status`                     | `{ session_key? }`                             | `{ ok, session }`                                              |                                                                                   |
| `runtime.status`                     | `{}`                                           | `{ ok, pid, daemon_protocol, daemon_build_id, channels, ... }` | Liveness + staleness probe.                                                       |
| `runtime.reload`                     | `{ ... }`                                      | `{ ok, ... }`                                                  | Reload runtime config; re-emits status.                                           |
| `slash`                              | `{ command }`                                  | `{ ok }`                                                       | Server-owned slash dispatch (aliases, plugins, skills).                           |
...
| `fetch_models`                       | `{ ... }`                                      | provider-specific                                              | Provider/model discovery.                                                         |
...
> Slash routing: the TS client handles a small set of client-local commands
> (`/help`, `/clear`, `/copy`, `/details`, `/logs`, `/queue`, ...) itself, and
> falls through to the `slash` method so Python keeps owning aliases, plugins,
> skills, and registry-backed commands.
...
(see the casing note in §1). The Python registry (`event_from_dict`) accepts
both spellings, and the stdio bridge transport emits PascalCase, so the table
below lists both. **The TS client keys on the snake_case column** and maps the
PascalCase column to the same handler as a tolerated alias.

### Event name map

| PascalCase (wire)  | snake_case (internal) | Payload fields                                                                          |
| ------------------ | --------------------- | --------------------------------------------------------------------------------------- |
