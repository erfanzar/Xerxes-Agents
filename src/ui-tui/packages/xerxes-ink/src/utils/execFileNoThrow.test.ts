...
beforeEach(() => {
  sleeperPids = []
  scriptDir = join(tmpdir(), `xerxes-execfile-test-${process.pid}-${Date.now()}`)
...
describe.skipIf(onWindows)('execFileNoThrow with daemon-style children', () => {
  ...
  it("settles immediately on 'exit' when resolveOnExit is true, regardless of daemon stdio", async () => {
    ...
    // (code 0) instead of waiting for the orphaned sleeper to release stdio
    // (which would block until the 4000ms timeout). We only need to prove we
    // returned on EXIT, not on TIMEOUT — the two paths are ~4s apart, so a
    // generous margin removes the flake a tight (<500ms) bound caused under
    // parallel test load without losing any signal.
