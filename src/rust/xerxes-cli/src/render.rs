...
fn draw_conversation(f: &mut Frame, app: &mut App, area: Rect, width: usize) {
    ...
    let max_scroll = u16::try_from(overflow).unwrap_or(u16::MAX);

    // When the buffer grows while the user is scrolled up, absorb the
    // growth into scroll_offset so their absolute reading position stays
    // put instead of drifting down as new lines arrive.
    if app.scroll_offset > 0 && max_scroll > app.max_scroll {
        let delta = max_scroll - app.max_scroll;
        app.scroll_offset = app.scroll_offset.saturating_add(delta).min(max_scroll);
    }

    // Publish the ceiling so keyboard/mouse handlers can clamp correctly.
    app.max_scroll = max_scroll;
    if app.scroll_offset > max_scroll {
        app.scroll_offset = max_scroll;
    }

    // `scroll_offset` counts lines *above the tail* we've rewound past.
    // The paragraph's own scroll arg counts lines hidden at the top, so
    // subtract the offset from the tail position.
    let scroll = max_scroll.saturating_sub(app.scroll_offset);

    let paragraph = Paragraph::new(wrapped).scroll((scroll, 0));
    f.render_widget(paragraph, area);

    // Scrollback indicator (top-right corner) while detached from the tail.
    if app.scroll_offset > 0 && area.height >= 1 && area.width >= 10 {
        let label = if app.scroll_offset >= max_scroll {
            " TOP ".to_string()
        } else {
...
pub fn welcome_lines(model: &str, provider: &str, _tools: usize, cwd: &str) -> Vec<Line<'static>> {
    let version = "v0.2.0";
    let title = format!(">_ Xerxes ({version})");
