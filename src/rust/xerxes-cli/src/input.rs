...
/// Scrollback actions usable from any mode that isn't actively consuming
/// arrow keys for a different purpose. The event loop translates these
/// into [`App::scroll_up`] / [`App::scroll_down`] calls.
...
/// Intercept viewport-scrollback keys before mode-specific handling.
/// Returns `Some(cmd)` when the key is a scroll gesture, else `None` so
/// the caller can fall through to its usual handler.
pub fn scroll_from_key(key: KeyEvent) -> Option<ScrollCmd> {
    match (key.modifiers, key.code) {
        (_, KeyCode::PageUp) => Some(ScrollCmd::PageUp),
        (_, KeyCode::PageDown) => Some(ScrollCmd::PageDown),
        (KeyModifiers::SHIFT, KeyCode::Up) => Some(ScrollCmd::LineUp),
        (KeyModifiers::SHIFT, KeyCode::Down) => Some(ScrollCmd::LineDown),
        (KeyModifiers::CONTROL, KeyCode::Home) => Some(ScrollCmd::ToTop),
        (KeyModifiers::CONTROL, KeyCode::End) => Some(ScrollCmd::ToBottom),
        _ => None,
    }
}
