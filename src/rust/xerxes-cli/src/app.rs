...
pub struct App {
    ...
    /// Lines currently hidden below the viewport bottom. 0 = follow-tail
    /// (classic behavior — stick to the latest output). Anything > 0 means
    /// the user has scrolled up and we should stop auto-snapping to the tail.
    pub scroll_offset: u16,
    /// Upper bound on `scroll_offset`, recomputed by the renderer each frame
    /// from the fully wrapped line count minus the visible height.
    pub max_scroll: u16,
}
...
impl App {
    ...
    /// Whether the viewport is currently pinned to the newest output.
    /// When true, new cells stay in view automatically.
    pub fn following_tail(&self) -> bool {
        self.scroll_offset == 0
    }

    /// Snap back to the latest output.
    pub fn scroll_to_bottom(&mut self) {
        self.scroll_offset = 0;
    }

    /// Jump to the oldest line currently held in the buffer.
    pub fn scroll_to_top(&mut self) {
        self.scroll_offset = self.max_scroll;
    }

    /// Scroll *up* by `n` lines (showing older content). Clamps to `max_scroll`.
    pub fn scroll_up(&mut self, n: u16) {
        self.scroll_offset = self.scroll_offset.saturating_add(n).min(self.max_scroll);
    }

    /// Scroll *down* by `n` lines (towards newer content). Clamps to 0.
