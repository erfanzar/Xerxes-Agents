...
use crossterm::{
    ...
    event::{
        ...
        EnableMouseCapture, Event as CEvent, MouseEventKind,
    },
    execute,
...
use input::{handle_key, scroll_from_key, Action, ScrollCmd};

#[derive(Parser)]
#[command(name = "xerxes", about = "Xerxes — agent CLI")]
struct Cli {
    #[command(subcommand)]
...
fn cmd_wakeup(
    ...
    if let Some(name) = profile {
        let code = format!(
            "from xerxes_agent.bridge.profiles import set_active; ok = set_active('{name}'); print('Switched to profile: {name}' if ok else 'Profile not found: {name}')"
        );
    ...
    if install {
        ...
        let output = std::process::Command::new(python)
            .args(["-c", "from xerxes_agent.daemon.service import install; print(install())"])
            .output()?;
        io::stdout().write_all(&output.stdout)?;
        io::stderr().write_all(&output.stderr)?;
        return Ok(());
    ...
    if uninstall {
        let output = std::process::Command::new(python)
            .args(["-c", "from xerxes_agent.daemon.service import uninstall; print(uninstall())"])
            .output()?;
        io::stdout().write_all(&output.stdout)?;
        return Ok(());
    }
    ...
    if stop {
        ...
        let pid_file = dirs_home().join(".xerxes/daemon/daemon.pid");
        if pid_file.exists() {
    ...
    if status {
        let output = std::process::Command::new(python)
            .args(["-c", "from xerxes_agent.daemon.service import status; print(status())"])
            .output()?;
        io::stdout().write_all(&output.stdout)?;
        return Ok(());
    }
    ...
    println!("Starting Xerxes daemon...");
    let status = std::process::Command::new(python)
        .args(["-m", "xerxes_agent.daemon", "--project-dir", project_dir])
        .stdin(Stdio::null())
        .status()?;
...
async fn cmd_send(prompt: &str, wait: bool) -> anyhow::Result<()> {
    let socket_path = dirs_home().join(".xerxes/daemon/xerxes_agent.sock");
    if !socket_path.exists() {
        anyhow::bail!(
            "Daemon not running (no socket at {}). Start it with `xerxes wakeup`.",
            socket_path.display()
        );
    }
...
async fn run_interactive(
    ...
    execute!(stderr, EnableBracketedPaste, EnableMouseCapture, cursor::Hide)?;
        ...
        tokio::select! {
            _ = tokio::task::yield_now() => {
                ...
                if event::poll(Duration::from_millis(poll_ms))? {
                    ...
                    CEvent::Mouse(me) => {
                        match me.kind {
                            MouseEventKind::ScrollUp => apply_scroll(app, ScrollCmd::LineUp),
                            MouseEventKind::ScrollDown => apply_scroll(app, ScrollCmd::LineDown),
                            _ => {}
                        }
                    }
                    CEvent::Key(key) => {
                        // Scroll-back gestures intercepted before mode-specific
                        // handlers. When in Input mode, these never carry
                        // printable chars so there's no conflict.
                        if let Some(cmd) = scroll_from_key(key) {
                            apply_scroll(app, cmd);
                        } else {
                        match handle_key(app, key) {
                            ...
                            Action::Submit(text) => {
                                ...
                                app.spinner.reset();
                                // New turn → snap back to the live tail.
...
/// Translate a scroll command into App mutations. Page size is a coarse
/// 10-line step — matches what most pagers do for PgUp/PgDn.
fn apply_scroll(app: &mut App, cmd: ScrollCmd) {
    const PAGE: u16 = 10;
    match cmd {
        ScrollCmd::LineUp => app.scroll_up(1),
        ScrollCmd::LineDown => app.scroll_down(1),
        ScrollCmd::PageUp => app.scroll_up(PAGE),
        ScrollCmd::PageDown => app.scroll_down(PAGE),
        ScrollCmd::ToTop => app.scroll_to_top(),
        ScrollCmd::ToBottom => app.scroll_to_bottom(),
    }
}
