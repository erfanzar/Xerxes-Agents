...
const BUILD_CONFIG_PATHS = new Set([
  ...
  'src/typescript/tsconfig.ui.json',
  'src/typescript/vitest.ui.config.ts',
])

const CRITICAL_SOURCE_PREFIXES = [
  'src/typescript/src/bridge/',
  'src/typescript/src/daemon/',
  'src/typescript/src/runtime/',
  'src/typescript/src/security/',
  'src/typescript/src/streaming/',
  'src/typescript/src/tools/',
] as const
...
function isTestFile(path: string): boolean {
  return (path.startsWith('src/typescript/test/') && /\.(test|spec)\.ts$/.test(path))
    || (path.startsWith('src/typescript/src/ui/') && /\.(test|spec)\.tsx?$/.test(path))
}

function isSourceFile(path: string): boolean {
  return path.startsWith('src/typescript/src/') && /\.tsx?$/.test(path)
}
