...
// (`method === "event"`). See `src/typescript/src/ui/PROTOCOL.md` for the frozen contract.
//
...
export function resolveBunDaemonEntry(projectDir: string, environment: Environment = process.env): string {
  ...
  const candidates = [
    join(projectDir, 'src', 'typescript', 'src', 'cli.ts'),
    join(projectDir, 'src', 'typescript', 'dist', 'cli.js'),
