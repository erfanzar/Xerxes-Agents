...
// Mirror the Bun daemon's lifecycle log from the TUI parent so breadcrumbs
// interleave, by timestamp, with the daemon's SIGTERM and exit entries.
//
...
const logDir = join(process.env.XERXES_HOME?.trim() || join(homedir(), '.xerxes'), 'logs')
...
// a real ~/.xerxes (tests must stay hermetic — see AGENTS.md).
