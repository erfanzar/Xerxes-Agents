...
export interface LegacySubagentNode {
  active: boolean
  agentId: string
...
export interface SubagentsState {
  byId: Record<string, LegacySubagentNode>
  order: string[]
}

export const emptySubagents: SubagentsState = { byId: {}, order: [] }

export function listSubagents(state: SubagentsState): LegacySubagentNode[] {
  return state.order.map(id => state.byId[id]).filter((node): node is LegacySubagentNode => Boolean(node))
}

export function subagentSummary(node: LegacySubagentNode): string {
  const lastLine = node.text
    .split('\n')
    .map(line => line.trim())
    .filter(Boolean)
    .at(-1)
  ...
  return node.active ? 'running' : 'done'
}

export function reduceSubagentEvent(state: SubagentsState, payload: Record<string, unknown>): SubagentsState {
  const id = String(payload.agent_id ?? payload.parent_tool_call_id ?? '').trim()
  if (!id) {
    return state
  }
  const event = payload.event && typeof payload.event === 'object' ? (payload.event as Record<string, unknown>) : {}
  const eventType = normalizeEventType(String(event.type ?? ''))
  const eventPayload =
  ...
  const current =
    ...
    ({
      active: true,
      agentId: id,
      parentToolCallId: String(payload.parent_tool_call_id ?? ''),
      subagentType: String(payload.subagent_type ?? ''),
      text: '',
      tools: []
    } satisfies LegacySubagentNode)
  const next: LegacySubagentNode = { ...current }

  if (eventType === 'text_part') {
    next.text += String(eventPayload.text ?? '')
    next.active = true
  } else if (eventType === 'think_part') {
    next.active = true
  } else if (eventType === 'tool_call') {
    next.tools = [...next.tools, String(eventPayload.name ?? 'tool')]
    next.active = true
  } else if (eventType === 'turn_end' || eventType === 'step_interrupted') {
    next.active = false
  } else if (eventType === 'turn_begin') {
    next.active = true
  }
