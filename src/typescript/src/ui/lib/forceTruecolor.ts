...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.XERXES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Xerxes on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via XERXES_TUI_TRUECOLOR=1.
