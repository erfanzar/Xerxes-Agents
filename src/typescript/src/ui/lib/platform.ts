 ...
 * Terminal decoders may set ``key.meta`` for plain Alt/Option on every
 * platform AND for Cmd on some legacy macOS terminals (Terminal.app without
 ...
 * (``xerxes_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but terminal decoders report Alt as ``key.meta`` on many
...
/** Match a normalized terminal key event against a parsed named key. The decoder
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS Alt may arrive as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` may be an Alt signal and accepting it would
