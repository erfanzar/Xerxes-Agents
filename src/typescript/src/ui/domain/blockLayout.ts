...
export const blockRenders = (
  msg: Pick<Msg, 'kind' | 'subagents' | 'thinking' | 'todos' | 'tools'>,
  ctx: DetailsCtx
): boolean => {
  if (msg.kind !== 'trail') {
    return true
  ...
  // Agent activity is rendered in the dedicated sidebar/overlay. A trail
  // carrying only an archived spawn tree is transparent to the transcript.
