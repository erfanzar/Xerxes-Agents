...
import type { RunExternalProcess } from '../lib/terminalTypes.js'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`xerxes ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching xerxes: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`xerxes ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
