...
// OpenTUI keeps painting from native scroll state.
...
export const resolveDeferredVirtualRange = ({
  deferredEnd,
  deferredStart,
  end,
  itemCount,
  ...
  const effectiveStart = start < deferredStart ? deferredStart : start
  const effectiveEnd = end > deferredEnd ? deferredEnd : end

  // A touching pair is empty just like an inverted pair. It can occur for a
  // single concurrent render when deferred start/end values come from
  // different range generations. Never let that transient hide every row.
  if (effectiveStart > effectiveEnd || (itemCount > 0 && effectiveStart === effectiveEnd) || sticky) {
    return [start, end]
  }
...
export function useVirtualHistory(
  ...
  // Callback refs are assigned during commit, after this hook has rendered.
  // A stable RefObject therefore cannot tell React that a conditionally
  // mounted scrollbox appeared (welcome -> first message) or was replaced.
  // Keep an explicit binding generation so useSyncExternalStore always
  // resubscribes to the current native scrollbox.
  const [scrollHandleVersion, bumpScrollHandleVersion] = useState(0)
  ...
  const setScrollHandle = useCallback(
    (handle: ScrollBoxHandle | null) => {
      if (scrollRef.current === handle) {
        return
      }

      scrollRef.current = handle
      bumpScrollHandleVersion(version => version + 1)
    },
    [scrollRef]
  )
  ...
  const subscribe = useCallback(
    (cb: () => void) => scrollRef.current?.subscribe(cb) ?? NOOP,
    // The generation deliberately participates even though it is not read in
    // the callback: changing it gives useSyncExternalStore a new subscribe
    // identity after the callback ref installs/replaces the native handle.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [scrollHandleVersion, scrollRef]
  )
  ...
  // Time-slice range growth via useDeferredValue. Urgent render keeps OpenTUI
  ...
  let [effStart, effEnd] = resolveDeferredVirtualRange({
    deferredEnd: dEnd,
    deferredStart: dStart,
    end,
    itemCount: n,
    start,
    sticky
