...
This is the frozen contract between the **TypeScript/OpenTUI frontend** (`src/typescript/src/ui/`)
and the **Bun TypeScript daemon** (`src/typescript/src/cli.ts daemon`). The
...
- Transport + framing: `src/typescript/src/daemon/server.ts`
- RPC dispatch + params: `DaemonServer.dispatch()` in that file
- Runtime/session lifecycle: `src/typescript/src/daemon/runtime.ts`
- UI connection bootstrap: `src/typescript/src/ui/gatewayClient.ts`

> **Stability:** the event-name map (snake_case ⇄ PascalCase) is asserted by a
> Bun contract test (`src/typescript/test/daemonServer.test.ts`) and the TUI
> gateway tests in `src/typescript/src/ui/__tests__/`. If you change an event name or
...
4. If not reachable, spawn the daemon and poll `runtime.status` until the
   ...
   ```sh
   bun src/typescript/src/cli.ts daemon \
     --project-dir <dir> \
     --socket <sock-path> \
     --pid-file <pid-path>
   ```
   ...
   | no daemon-path setting | A colocated `src/typescript/src/cli.ts` or built `src/typescript/dist/cli.js` |
...
| `runtime.status`                     | `{}`                                           | `{ ok, runtime_ready, pid, daemon_protocol, daemon_build_id, channels, ... }` | Liveness probe; `runtime_ready` reports configured-provider readiness. |
