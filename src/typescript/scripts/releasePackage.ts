...
const SOURCE_ARTIFACTS = Object.freeze([
  {
    ...
    kind: "file",
    source: "src/typescript/dist/cli.js",
  },
  {
    destination: "ui/entry.js",
    kind: "file",
    source: "src/typescript/dist/ui/entry.js",
  },
  ...
  },
  {
    ...
    kind: "directory",
    source: "src/typescript/dist/skills",
  },
  {
    destination: BUNDLED_AGENTS_DIRECTORY,
    kind: "directory",
    source: "src/typescript/dist/default",
  },
] as const);
...
export async function validateReleasePackage(
  ...
  for (const launcher of ["xerxes", "xerxes-acp"]) {
...
export function parseReleasePackageCommand(
  ...
  if (kind === "check" && packageDirectory === undefined) {
    throw new Error("check requires --package <directory>.");
  }
  if (
    kind === "prepare" &&
  ...
  if (kind === "check" && outputDirectory !== undefined) {
    throw new Error(
      "check only accepts --package, --archive, and --expected-version.",
    );
  }
  return {
    archivePath,
...
async function releaseVersion(
  ...
  const packagePaths = ["package.json", "src/typescript/package.json"];
...
async function packageMetadata(
  ...
  const runtimeManifest = assertRecord(
    await readJson(join(repositoryRoot, "src/typescript/package.json")),
...
function assertReleasePackageMetadata(value: unknown, version: string): void {
  ...
  if (metadata.private !== true)
    throw new Error(
      "Release package must remain private until registry publishing is explicitly enabled.",
    );
  ...
  if (
    ...
    !files.includes(BUNDLED_SKILLS_DIRECTORY)
  ) {
    throw new Error("Release package metadata must include bundled skills.");
  }
...
async function assertPackedArchive(
  ...
  for (const launcher of ["bin/xerxes", "bin/xerxes-acp"]) {
