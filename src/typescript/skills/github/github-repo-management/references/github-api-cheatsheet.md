...
Prefer an existing authenticated `gh` session. Direct REST examples require a
`GITHUB_TOKEN` explicitly injected by the host; never recover one from local
credential files or print it. Derive owner/repository names from the Git remote
without touching authentication state.
