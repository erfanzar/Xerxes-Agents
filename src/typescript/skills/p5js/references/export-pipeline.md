...
The `noLoop()` + `redraw()` pattern allows a user or approved host adapter to advance one frame at a time without racing the browser's draw loop.
...
function draw() {
  // This runs only when redraw() is called deliberately
  // frameCount increments exactly once per redraw()
}
```

Xerxes does not ship a capture launcher. Automation may attach only through the native browser integration to an already-running endpoint explicitly supplied by the user.
...
```
project/
├── capture-scene.js          # Shared: bun capture-scene.js <html> <outdir> <frames>
...
```bash
bun capture-scene.js scenes/01-particles.html clips/01-particles 150
...
## Automated Preview and Capture

Do not launch Chromium, Puppeteer, or Playwright from a bundled asset. Do not disable sandboxing or browser security controls. If the user requests automation, attach through Xerxes' native browser integration to an existing CDP endpoint the user explicitly supplied. Otherwise, keep preview and export browser-native and user-controlled.
...
| Social media video | MP4 | User-created frames + user-owned ffmpeg |
...
| Approved automated capture | PNG/MP4 | Native browser integration + explicit existing endpoint |
...
// Build: bunx --bun fxhash build → upload.zip
// Dev: bunx --bun fxhash dev → localhost:3300
```
