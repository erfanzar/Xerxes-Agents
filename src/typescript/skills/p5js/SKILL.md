---
name: p5js
description: "Production pipeline for interactive and generative visual art using p5.js. Creates browser-based sketches, generative art, data visualizations, interactive experiences, 3D scenes, audio-reactive visuals, and motion graphics — exported as HTML, PNG, GIF, MP4, or SVG. Covers: 2D/3D rendering, noise and particle systems, flow fields, shaders (GLSL), pixel manipulation, kinetic typography, WebGL scenes, audio analysis, mouse/keyboard interaction, and high-resolution browser-native export. Use when users request: p5.js sketches, creative coding, generative art, interactive visualizations, canvas animations, browser-based visual art, data viz, shader effects, or any p5.js project."
version: 1.0.0
metadata:
  xerxes:
...
5. **EXPORT** — Capture output with browser-native `saveCanvas()`, `saveGif()`, or `saveFrames()`; encode a user-created frame sequence with an explicitly available ffmpeg installation
...
- For `loadImage()`/`loadFont()` from local files, ask the user to provide an already-running local server or an explicit preview URL
...
| **High-res PNG** | Render a larger canvas and call `saveCanvas()` | Press 's' to save |
...
| **MP4** | Browser-created frame sequence + user-owned ffmpeg | Encode only after the user supplies the frames and encoder |
...
Xerxes does not bundle or launch a browser, disable a browser sandbox, or ship a capture script. Browser automation may attach only through the native browser integration to an endpoint the user explicitly supplied and already owns.

For deterministic user-controlled capture, stop the automatic loop and advance frames deliberately:

```javascript
function setup() {
  createCanvas(1920, 1080);
  pixelDensity(1);
  noLoop();                    // user or an approved host adapter advances frames
}
```

Do not introduce Puppeteer, Playwright, shell launchers, or an ambient browser process as a fallback. See `references/export-pipeline.md` for browser-native export patterns.
...
3. **Local assets** (fonts, images) require an explicit URL from a user-owned server; do not start a server automatically
4. **Export PNG/GIF** — add `keyPressed()` shortcuts as shown above, tell the user which key to press
5. **Automated preview** — only attach through the native browser integration when the user supplies an existing endpoint
6. **MP4 rendering** — encode only a user-created frame sequence with a user-approved ffmpeg installation
...
| `references/export-pipeline.md` | `saveCanvas()`, `saveGif()`, `saveFrames()`, user-controlled ffmpeg frame-to-video, CCapture.js, SVG export, per-clip architecture, platform export (fxhash), video gotchas |
