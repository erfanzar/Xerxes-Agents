...
test("the production image keeps the built-in agent profiles beside the bundled CLI", async () => {
  ...
  expect(dockerfile).toContain(
    "/app/src/typescript/dist/default ./src/typescript/dist/default",
  );
  expect(dockerfile).toContain(
    "/app/src/typescript/dist/skills ./src/typescript/dist/skills",
  );
});
...
test("the production image installs only the locked OpenTUI runtime graph", async () => {
  ...
  const sourceManifest = await Bun.file(
    join(REPOSITORY_ROOT, "src/typescript/package.json"),
  ).json();
  ...
  expect(dockerfile).not.toContain(
    "COPY --from=build /app/src/typescript/package.json",
  );
  expect(dockerfile).toContain(
