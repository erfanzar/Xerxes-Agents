...
describe("native release package staging", () => {
  test("stages a locked Bun package with an executable launcher and integrity manifest", async () => {
    ...
    expect(metadata).toMatchObject({
      bin: { xerxes: "./bin/xerxes", "xerxes-acp": "./bin/xerxes-acp" },
      dependencies: {
        "@opentui/core": "0.4.3",
        "@opentui/react": "0.4.3",
        react: "^19.2.4",
      },
      files: expect.arrayContaining(["skills"]),
  ...
  test("accepts a clean-room single-file PowerPoint skill", async () => {
    ...
    const skillPath = join(
      repositoryRoot,
      "src/typescript/dist/skills/powerpoint/SKILL.md",
    );
...
async function releaseFixture(): Promise<string> {
  ...
  await mkdir(join(repositoryRoot, "src/typescript/dist"), { recursive: true });
  await mkdir(join(repositoryRoot, "src/typescript/dist/default"), {
    recursive: true,
  });
  await mkdir(join(repositoryRoot, "src/typescript/dist/ui"), {
    recursive: true,
  });
  await mkdir(
    join(repositoryRoot, "src/typescript/dist/skills/fixture-skill/references"),
    { recursive: true },
  );
  ...
  await Promise.all([
    ...
    writeFile(
      join(repositoryRoot, "src/typescript/package.json"),
      runtimePackageManifest,
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/cli.js"),
      "console.log(JSON.stringify(Bun.argv.slice(2)))\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/agent.yaml"),
      "version: 1\nagent:\n  name: default\n  system_prompt_path: ./system.md\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/coder.yaml"),
      "version: 1\nagent:\n  name: coder\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/objective.yaml"),
      "version: 1\nagent:\n  name: objective\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/planner.yaml"),
      "version: 1\nagent:\n  name: planner\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/researcher.yaml"),
      "version: 1\nagent:\n  name: researcher\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/default/system.md"),
      "You are Xerxes.\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/skills/fixture-skill/SKILL.md"),
    ...
    writeFile(
      join(
        repositoryRoot,
        "src/typescript/dist/skills/fixture-skill/references/guide.md",
      ),
      "Bundled release skill asset.\n",
      "utf8",
    ),
    writeFile(
      join(repositoryRoot, "src/typescript/dist/ui/entry.js"),
      'console.log("tui")\n',
      "utf8",
    ),
  ...
  await chmod(join(repositoryRoot, "src/typescript/dist/cli.js"), 0o644);
