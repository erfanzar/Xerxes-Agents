...
test('change guard classifies risky porcelain rows, extracts verification evidence, and uses an injected git runner', async () => {
  const report = analyzeStatusLines([
    ' D src/typescript/test/standaloneTools.test.ts',
    ' M src/typescript/src/tools/standalone.ts',
    ' M src/typescript/src/runtime/queryEngine.ts',
  ], [{
    name: 'exec_command',
  ...
  const throughRunner = await analyzeWorkspaceChanges('/workspace', [], {
    commandRunner: async args => {
      ...
      return { exitCode: 0, stdout: ' M src/typescript/src/security/policy.ts\n' }
    },
  })
