...
- **File-based definitions**: Load agent definitions from ``.md`` files
  with YAML frontmatter, supporting user-level (``~/.calute/agents/``)
  and project-level (``.calute/agents/``) overrides.
...
Usage::
    ...
    from calute.agents.definitions import (
        get_agent_definition,
        load_agent_definitions,
        BUILTIN_AGENTS,
    )

    # Get a built-in definition
...
BUILTIN_AGENTS: dict[str, AgentDefinition] = {
    ...
    "researcher": AgentDefinition(
        ...
        tools=["Read", "ReadFile", "Glob", "Grep", "ListDir", "DuckDuckGoSearch"],
        source="built-in",
    ),
...
def load_agent_definitions(
    ...
    - ``~/.calute/agents/*.md`` (user-level, overrides built-ins).
    - ``.calute/agents/*.md`` (project-level, overrides user).
    ...
    # User-level
    udir = user_dir or Path.home() / ".calute" / "agents"
    if udir.is_dir():
    ...
    pdir = project_dir or Path.cwd() / ".calute" / "agents"
    if pdir.is_dir():
