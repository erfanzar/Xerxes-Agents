...
Usage::
    ...
    from calute.agents.subagent_manager import SubAgentManager
    from calute.agents.definitions import get_agent_definition
    ...
    # Send a follow-up while it's running
...
def _create_worktree(base_dir: str) -> tuple[str, str]:
    ...
    branch = f"calute-agent-{uuid.uuid4().hex[:8]}"
    wt_path = tempfile.mkdtemp(prefix="calute-agent-wt-")
    os.rmdir(wt_path)
    subprocess.run(
...
def _has_worktree_changes(wt_path: str) -> bool:
    ...
    try:
        ...
        if r.stdout.strip():
            return True
        # Check if there are commits beyond the parent
        r2 = subprocess.run(
...
class SubAgentManager:
    ...
    def __init__(self, max_concurrent: int = 5, max_depth: int = 5):
        ...
        self._agent_runner: Any = None  # Set externally: callable(prompt, config, system_prompt, depth, cancel_check)
    ...
    def spawn(
        ...
        # Build effective config and system prompt
        ...
        if agent_def:
            ...
            # Override isolation from agent_def if not specified
        ...
        def _run() -> None:
            ...
            try:
                ...
                if runner:
                    ...
                    # Fallback: use the streaming loop directly
                    task.result = _run_streaming_loop(prompt, eff_config, eff_system, depth + 1, task)
                ...
                # Drain inbox: process follow-up messages
                while not task._inbox.empty() and not task._cancel_flag:
                    ...
                    if runner:
                        ...
                        task.result = _run_streaming_loop(inbox_msg, eff_config, eff_system, depth + 1, task)
                ...
                if worktree_path:
                    os.chdir(old_cwd)
                    # Only cleanup if no changes were made
