...
Usage::
    ...
    from calute.streaming.permissions import PermissionMode, check_permission

    # Auto mode — Read is safe, Edit is not
    ...
    # Accept-all mode — everything is approved
...
# Bash patterns that are always dangerous
...
def check_permission(
    ...
    # ExecutePythonCode — safe if read-only (no file writes, no subprocess)
    if name == "ExecutePythonCode":
        code = tool_call.get("input", {}).get("code", "")
        # Heuristic: block if it contains file write or subprocess patterns
        if re.search(r"\b(open\(.*['\"]w|subprocess|os\.system|os\.popen|shutil)\b", code):
            return False
        return True

    # Agent / SendMessage — generally safe (they run in their own sandbox)
    if name in ("Agent", "SendMessage"):
        return True

    # Memory save/delete — low risk
    if name in ("MemorySave", "MemoryDelete"):
        return True

    # Known write tools — ask permission
    if name in ("Write", "WriteFile", "Edit", "FileEditTool", "AppendFile"):
        return False

    # Everything else (unknown/hallucinated tool names) — auto-approve
    # They'll just return "Unknown tool" from the executor, which is fine
    return True
