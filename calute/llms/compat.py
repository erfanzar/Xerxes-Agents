...
The class reads provider metadata from :mod:`calute.llms.registry` and
resolves API keys from environment variables automatically.

Usage::

    from calute.llms.compat import OpenAICompatLLM
...
class OpenAICompatLLM(OpenAILLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Resolve api_key from registry/env if not explicitly set
        if not config.api_key and prov:
            config.api_key = get_api_key(self.provider_name)

        # Set context limit from registry
